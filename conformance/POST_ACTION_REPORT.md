...
| Multitenancy | Released v1.1.113 owned evidence runs the current two-principal/two-organization matrix, normalized issuer-relationship authorization, the independent DIDComm positive case, cross-tenant DIDComm transaction substitution, and shipped-browser organization/policy substitutions. It covers membership/RBAC, template/policy/DID substitution, API-key binding, SCIM, flows/results, issuance/revocation, trust ownership and decisions, applicant/evidence, deployment/device, webhook, wallet, notification/SSE, audit, custody-selector rejection, unknown/inactive/incompatible DID denial, idempotent uniqueness, forced ambiguity rejection/recovery, and non-leaking denials. Live `TRUSTED` to `UNDER_REVIEW`, below-threshold, and restored relationship transitions affect the immediately following public verification | External trust-registry import/synchronization and a first-class multiple-accreditation model remain owned gaps under [marty-integration-tests#224](https://github.com/ElevenID/marty-integration-tests/issues/224) |
...
| Wider Marty feature model | Official suites do not cover it; the ElevenID-owned released matrix now covers membership/RBAC, API keys, SCIM, saved verification flows/results, issuance and revocation, organization trust-profile ownership and normalized issuer decisions, applicant/vetting and evidence, deployments/devices, webhooks, wallet profiles, notification/SSE delivery, audit, DID resolution, forced ambiguity, independent DIDComm decoding, DIDComm cross-tenant substitution, and adversarial shipped-browser organization/policy substitution | External trust-registry synchronization, multiple-accreditation semantics, and broader DIDComm modes remain owned work under [marty-integration-tests#224](https://github.com/ElevenID/marty-integration-tests/issues/224) |
