...
- The OIDF mdoc lane remains a genuine visible failure. All three positive
  ...
  an invalid device signature. Marty rejected only the upstream fixture's
  `CA:true`/`keyCertSign,cRLSign` certificate because ISO 18013-5 Table B.3
  requires a non-CA document signer with `digitalSignature`. OIDF tracks this
  in [work item 1891](https://gitlab.com/openid/conformance-suite/-/work_items/1891).
  No product trust rule, official assertion, or expected result was weakened.
...
and independent DIDComm decryption plus tamper rejection. Its summary records
`official_suite_invoked=false` and `official_suite_source_modified=false`.

The public contract is MIP v0.4.1 at Marty-Protocol commit
`6387a02fe4a6eaa08b4743cae97590adee11190c`. Trust Profile lifecycle status is
now a required canonical lowercase enum across schema fixtures and generated
Python, Rust, and TypeScript bindings. The released runtime matches the exact
public Trust Profile fields and requiredness, including revocation and time
policy fields, and exposes no stale validation or custody-routing members.

The owned boundary lane is not an official standards-conformance result. The DIDComm
...
not a public selector. Marty-Protocol MIP v0.4.1 now defines the complete
covered public boundary, including canonical Trust Profile status. Generated
Python, Rust, and TypeScript bindings are freshness-tested, and the Python
gateway and JavaScript UI are mechanically compared with those schemas for
fields, requiredness, nullability, and conditional response members. Public
requests still pass through fail-closed public-to-internal adapters rather
than exposing private resolver or custody state. The remediation tracked by
[marty-ui#222](https://github.com/ElevenID/marty-ui/issues/222) is closed and
...
`did:peer:2` recipient, and CA-validated HTTPS. Immutable run
[31212182823](https://github.com/ElevenID/marty-integration-tests/actions/runs/31212182823)
also requires the released Marty binding and pinned Go implementation to both
reject independently mutated ciphertext, authentication tag,
integrity-protected header, and wrapped content-encryption key. Artifact
`9007217423` has digest
`sha256:4d6ab1420f111b35dfbab9823e28518e269c14f589800cf5090e9e2452a1005b`.
It does not prove independent full-agent exchange, authcrypt/signed modes,
inbound behavior, every algorithm or DID method, mediator/routing behavior,
multiple recipients, broader message-validation negatives, or the complete
Issue Credential state machine. This owned lane must not be presented as an
...
### 15. Normalized issuer relationships existed but did not authorize verification

The audit found that `TrustProfile`-to-`IssuerEntity` relationships were
tenant-isolated management records, but the production presentation evaluator
did not use them. It still decided issuer trust from the older denormalized
`allowed_issuers`, `denied_issuers`, and trust-source fields. Public
`issuer_constraints` such as minimum trust level and required compliance status
were serialized but did not affect the decision. A relationship could therefore
be changed to `DENIED` or `UNDER_REVIEW` without changing verification behavior.

[marty-ui#303](https://github.com/ElevenID/marty-ui/pull/303) makes the internal
trust-profile view materialize the exact linked issuer identifier, trust level,
relationship and compliance status, validity interval, accreditation authority,
and revocation time. The evaluator now requires an active profile and exactly
one exact normalized issuer relationship, without collapsing distinct
`did:web` paths on the same domain. It fails closed for missing, ambiguous,
denied, under-review, suspended, revoked, future, expired, malformed,
below-threshold, noncompliant, or unaccredited relationships. Legacy lists are
used only when no normalized relationship exists and the policy does not require
normalized constraints.

The same review found that a five-minute process-local cache could preserve an
old allow decision after a relationship changed. The authorization cache and
its deprecated configuration were removed. Every evaluation now reads the
current tenant-bound internal trust decision; unit coverage proves a
`TRUSTED`-to-`DENIED` change takes effect on the next call.

Immutable stack v1.1.113 published these changes through release run
[31215275314](https://github.com/ElevenID/marty-ui/actions/runs/31215275314),
commit `c6a84266aad557a23e4fbea7ecf71b382dc5fec7`, and manifest
`sha256:fff946d9fb58b31f5c13e57692821d4246c4728f841bfbc76f8719a772e7b61a`.
The ElevenID-owned production-boundary run
[31215980125](https://github.com/ElevenID/marty-integration-tests/actions/runs/31215980125)
then issued and redeemed a real organization-B SD-JWT credential, registered
its actual signing DID as an issuer entity, linked it to an active trust
profile, and evaluated it through the public Presentation Policy API. The
released stack allowed the trusted relationship, denied the immediately
following `UNDER_REVIEW` evaluation, denied trust level 79 against a minimum of
80, and allowed the credential again after restoration. The remainder of the
two-principal/two-organization, shipped-browser, and independent DIDComm matrix
also passed. Artifact `9008612765` has digest
`sha256:bd299222386de9e70c1c91b744364de3dadf157fba9239b86ff4a766c3fab94e`;
its summary is
`sha256:6029a944c9622645e109706aa7bfba212973483f4fe23230d555fc4218e6010a`.
This is explicitly ElevenID-owned product-security evidence: no imported
official suite was invoked or modified for this assertion.

### 16. Accreditation authority was mistaken for accreditation evidence

The normalized trust decision initially exposed only `accreditation_body`.
The evaluator treated that certifying authority as though it were the set of
accreditations held by the issuer. A policy requiring a named accreditation
could therefore be satisfied by an authority with the same text, and there was
no first-class way to require multiple independent accreditation identifiers.

[Marty-Protocol#27](https://github.com/Marty-Protocol/Marty-Protocol/pull/27)
separates the authority from a bounded `accreditations` set in the public
IssuerEntity contract and regenerated bindings. The contract trims identifiers,
rejects blanks and case-insensitive duplicates, and defines require-all,
case-insensitive comparison. [marty-ui#305](https://github.com/ElevenID/marty-ui/pull/305)
persists that set, preserves complete-replacement update semantics, propagates
it through the strict gateway and UI models, and makes malformed or incomplete
evidence fail closed. `accreditation_body` never counts as an accreditation.

The first release attempt also exposed a fresh-install migration defect. The
new Alembic revision tried to alter the runtime-created `issuer_entities` table
before service startup. Release run
[31221844063](https://github.com/ElevenID/marty-ui/actions/runs/31221844063)
failed in the artifact-only public-stack smoke and published no GitHub release.
[marty-ui#307](https://github.com/ElevenID/marty-ui/pull/307) makes the migration
safe for both database states: a fresh install records the revision and lets the
current model create the complete table, while an existing deployment receives
the non-null, fail-closed `[]` column only when it is missing. The failed
`v1.1.114` tag was not moved or reused.

Immutable `marty-ui` v1.1.115 then passed release run
[31223111668](https://github.com/ElevenID/marty-ui/actions/runs/31223111668),
including provenance validation, signed UI/services/migrations images, SBOMs,
no-commerce checks, upgrade/rollback roles, and the same fresh-database public
stack. Its release commit is
`06392419af769b79eb6199d3913c355c7a251049` and its manifest is
`sha256:25df3a37476bc30ea08f8b58f0664bf84a373512d06de26ddf9a785510fcbe69`.

The ElevenID-owned production-boundary run
[31223622247](https://github.com/ElevenID/marty-integration-tests/actions/runs/31223622247)
then registered `ISO27001` and `FIPS140-2` on the real signing DID and required
both with mixed casing. Verification passed, then failed immediately after
`FIPS140-2` was removed even though `accreditation_body` was deliberately set
to `FIPS140-2`, and passed again after the evidence set was restored in a
different order and case. The full two-organization, shipped-browser, DIDComm
cross-implementation, and tamper-rejection matrix remained green. Harness
commit `96ccd72585f16b318f4c52a7b6c14b77f9868bb1` produced artifact
`9011371914`, digest
`sha256:5277b7a6ded427b7d5b1f36d80c806bc92f54a2ebfdb4eae73e761998ec9d350`,
and summary
`sha256:c3b1870edcc0ac916d79b358f095ccb6916bc4b42bc3f11e389a33a90dcaeff5`.
This remains explicitly ElevenID-owned product-security evidence; no imported
official suite was invoked or modified.
...
This closes the released-browser issuance and verification slice, including
adversarial organization and presentation-policy substitution. Exact immutable
run [31210134281](https://github.com/ElevenID/marty-integration-tests/actions/runs/31210134281)
mutated the actual requests emitted by the shipped UI, required non-enumerating
denials, then completed the normal positive issuance and signed-verification
controls. The generated-client and runtime contract drift work under
[marty-ui#222](https://github.com/ElevenID/marty-ui/issues/222) is complete and
remains enforced against the exact protocol pin. The official suite itself
remains API-driven and unmodified; this browser evidence remains separately
labeled ElevenID-owned product-security evidence.
...
| Protocol contract | Public DID-only Credential Template, Organization Trust Profile, issuance and issued-credential lifecycle, verification-flow start/resource/execution/result, complete Presentation Policy and Organization operations, strict IssuerEntity trust-record/accreditation operations, and the DID-only IssuerIdentity lifecycle are merged. Main CI and the E2E repository variable pin Marty-Protocol MIP v0.4.1 commit `6387a02fe4a6eaa08b4743cae97590adee11190c`; schema parity and generated Python/Rust/TypeScript freshness pass, canonical Trust Profile status is enforced, raw-model/private-field bypasses are rejected, and cross-tenant or global/system mutation fails closed | Retain the single exact cross-repository pin, generated-binding checks, and public/internal adapter separation |
| Wider Marty feature model | Official suites do not cover it; the ElevenID-owned released matrix covers membership/RBAC, API keys, SCIM, saved verification flows/results, issuance and revocation, organization trust-profile ownership, normalized issuer decisions, multiple-accreditation semantics, atomic external trust-registry synchronization/rollback, applicant/vetting and evidence, deployments/devices, webhooks, wallet profiles, notification/SSE delivery, audit, DID resolution, forced ambiguity, independent DIDComm decoding, DIDComm cross-tenant substitution, and adversarial shipped-browser organization/policy substitution | Broader DIDComm modes and future non-standard product capabilities remain owned work under [marty-integration-tests#224](https://github.com/ElevenID/marty-integration-tests/issues/224) |

### Exposed-gap action ledger

| Finding | Classification | Impact | Owner/remediation | Status and required evidence |
| --- | --- | --- | --- | --- |
| Normalized issuer relationships were decorative and authorization decisions were stale | Product authorization / trust-registry defect | `TrustProfile`-to-`IssuerEntity` status, trust level, compliance, validity, accreditation, and revocation did not control verification; a cached allow could survive a relationship change for five minutes | [marty-ui#303](https://github.com/ElevenID/marty-ui/pull/303) materializes a tenant-bound internal decision view, enforces exactly one current relationship and policy constraints, uses exact DID matching, and removes authorization caching. [marty-integration-tests#289](https://github.com/ElevenID/marty-integration-tests/pull/289) exercises the actual signing DID and credential through public issuance and verification | Merged and released in immutable v1.1.113. Run [31215980125](https://github.com/ElevenID/marty-integration-tests/actions/runs/31215980125) proves trusted allow, immediate under-review denial, minimum-trust denial, and restored allow against the exact released manifest; the full tenant, browser, and DIDComm controls remain green. No imported official suite changed |
| Accreditation authority and held accreditation identifiers were conflated | Product authorization / public contract / fresh-migration defect | A required accreditation could be satisfied by identically named certifying-authority text; multiple accreditation evidence was not representable; and the first additive migration rolled back fresh public-stack installation because its runtime table did not yet exist | [Marty-Protocol#27](https://github.com/Marty-Protocol/Marty-Protocol/pull/27) separates and defines the public fields; [marty-ui#305](https://github.com/ElevenID/marty-ui/pull/305) persists and enforces them; [marty-ui#307](https://github.com/ElevenID/marty-ui/pull/307) repairs fresh/existing migration sequencing; [marty-integration-tests#290](https://github.com/ElevenID/marty-integration-tests/pull/290) adds released evidence | Failed release run [31221844063](https://github.com/ElevenID/marty-ui/actions/runs/31221844063) published no release. Immutable v1.1.115 release [31223111668](https://github.com/ElevenID/marty-ui/actions/runs/31223111668) and boundary run [31223622247](https://github.com/ElevenID/marty-integration-tests/actions/runs/31223622247) pass. Removal denies even when the authority matches the missing identifier; case/order restoration allows. No official suite changed |
| Trust Profile lifecycle status was optional and unconstrained in the public schema while the runtime required a canonical value | Protocol drift / authorization-state ambiguity | Generated clients could omit status or send inconsistent casing and vocabulary even though production authorization depends on lifecycle state | [Marty-Protocol#29](https://github.com/Marty-Protocol/Marty-Protocol/pull/29) publishes required `draft`, `active`, `suspended`, and `archived` values in MIP v0.4.1; [marty-cli#12](https://github.com/ElevenID/marty-cli/pull/12) and [marty-ui#317](https://github.com/ElevenID/marty-ui/pull/317) align generated clients and runtime models; [marty-integration-tests#295](https://github.com/ElevenID/marty-integration-tests/pull/295) uses the canonical value in owned evidence | Released in protocol v0.4.1, CLI/API core v0.2.3, immutable stack v1.1.122, and integration harness v1.2.41. All protocol generation/parity checks, release gates, official lanes except the independently tracked upstream mdoc fixture, and complete owned boundary run 31237661261 execute against the aligned contract. No imported official suite changed |
...
| Published public operation contracts and production models were not mechanically coupled | Protocol drift | A repository could add a public custody selector, leak internal routing, or change required request/response fields while both repositories remained independently green | Marty-Protocol [#13](https://github.com/Marty-Protocol/Marty-Protocol/pull/13) through [#29](https://github.com/Marty-Protocol/Marty-Protocol/pull/29) define the DID-first core, policy, organization, issuer, Trust Profile, Flow, issuance, and issued-credential contracts; marty-ui [#220](https://github.com/ElevenID/marty-ui/pull/220) through [#317](https://github.com/ElevenID/marty-ui/pull/317) align and enforce the runtime boundary; [marty-credentials#89](https://github.com/ElevenID/marty-credentials/pull/89) enforces the authoritative lifecycle boundary | Exact MIP v0.4.1 protocol commit `6387a02fe4a6eaa08b4743cae97590adee11190c` is pinned in code and repository variables. The gate covers strict request/response fields, requiredness, canonical partial updates, tenant binding, conditional serialization, lifecycle status, and recursive private-state rejection across every audited family. Generated binding freshness and runtime parity are enforced, and [marty-ui#222](https://github.com/ElevenID/marty-ui/issues/222) is closed. No imported official suite, runner, fixture, vector, assertion, expected result, exclusion, or test selection changed |
...
| Official lanes do not drive the browser | Missing evidence | API-only official lanes could not prove UI request shapes, private-selector absence, or browser-origin tenant substitution denial | Released-stack Playwright evidence in run 31210134281 selected public DIDs, mutated actual approval/verification requests with foreign organization and policy values, required non-enumerating denial, and completed the positive issuance and signed-verification controls | Current owned browser slice complete; keep it green as the released UI changes, while retaining the explicit non-official evidence label |
...
| Adversarial shipped-browser boundary run [31210134281](https://github.com/ElevenID/marty-integration-tests/actions/runs/31210134281), artifact ID `9006459897`, digest `sha256:18fd12ca445cfa5263cad121839299177b1ac8b267aa5a500c52c691ae440b02` | Exact v1.1.112 and harness `78fcaf04f269b38c3a79bce7c8a4a41f6baad7bd` passed in immutable-release mode. The real UI emitted valid organization-A approval and verification requests; Playwright substituted organization B at approval, a B-owned presentation policy at verification, and organization B at verification. Every mutation received a non-enumerating denial, then the original issuance and signed-verification positive controls passed. The same run retained the full two-organization matrix and independent DIDComm decryption. Evidence records `official_suite_invoked=false` and `official_suite_source_modified=false`. |
| DIDComm cryptographic tamper-rejection run [31212182823](https://github.com/ElevenID/marty-integration-tests/actions/runs/31212182823), artifact ID `9007217423`, digest `sha256:4d6ab1420f111b35dfbab9823e28518e269c14f589800cf5090e9e2452a1005b` | Exact v1.1.112 and harness `0f1eb65bf57add3f75c00900997335232dc99d84` passed in immutable-release mode. After successful independent decryption and semantic plaintext equality, both the attested released Marty binding and pinned `notabene-id/go-didcomm v0.4.0` rejected four separately mutated envelopes: ciphertext, authentication tag, integrity-protected header, and wrapped content-encryption key. The complete two-organization and shipped-browser matrix remained green. Evidence explicitly records `official_suite_invoked=false` and `official_suite_source_modified=false`; this is selected-profile cross-implementation security evidence, not certification or a full agent exchange. |
...
| Normalized issuer-decision production run [31215980125](https://github.com/ElevenID/marty-integration-tests/actions/runs/31215980125), artifact ID `9008612765`, artifact digest `sha256:bd299222386de9e70c1c91b744364de3dadf157fba9239b86ff4a766c3fab94e`, summary `sha256:6029a944c9622645e109706aa7bfba212973483f4fe23230d555fc4218e6010a` | Exact v1.1.113 manifest `sha256:fff946d9fb58b31f5c13e57692821d4246c4728f841bfbc76f8719a772e7b61a`, release commit `c6a84266aad557a23e4fbea7ecf71b382dc5fec7`, and owned harness `4a2c64d5a8ff134632c5c4aea4cb7cee3c58d9a2` passed in immutable-release mode. A real organization-issued and wallet-redeemed SD-JWT was allowed through its exact normalized issuer-DID relationship, denied immediately after `UNDER_REVIEW`, denied at trust level 79 against minimum 80, and allowed after restoration. Full multitenancy, shipped-browser, cross-tenant DIDComm, independent decryption, and tamper-rejection controls remained green. Evidence explicitly records that no official suite was invoked or modified. |
| Accreditation evidence release [31223111668](https://github.com/ElevenID/marty-ui/actions/runs/31223111668) and production-boundary run [31223622247](https://github.com/ElevenID/marty-integration-tests/actions/runs/31223622247), artifact ID `9011371914`, artifact digest `sha256:5277b7a6ded427b7d5b1f36d80c806bc92f54a2ebfdb4eae73e761998ec9d350`, summary `sha256:c3b1870edcc0ac916d79b358f095ccb6916bc4b42bc3f11e389a33a90dcaeff5` | Exact v1.1.115 manifest `sha256:25df3a37476bc30ea08f8b58f0664bf84a373512d06de26ddf9a785510fcbe69`, release commit `06392419af769b79eb6199d3913c355c7a251049`, and owned harness `96ccd72585f16b318f4c52a7b6c14b77f9868bb1` passed in immutable-release mode. Two required accreditations match case-insensitively; removing one denies the next verification even when `accreditation_body` equals the missing identifier; restoring the evidence in different case/order allows. The full multitenancy, browser, independent DIDComm, and tamper matrix remained green. Evidence records `official_suite_invoked=false` and `official_suite_source_modified=false`. |
...
- [x] all supported public clients consume or mechanically compare the generated
