...
def configure_haip_environment(
    ...
    *,
    ...
    if has_certificate:
        ...
        validated = validate_verifier_environment(certificate)
    ...
    if material_dir is None:
        raise ValueError("--haip requires external VERIFIER_X509_CERT_PEM or --haip-material")
...
def stage_haip_profile_certificate(
    ...
    if args.haip_material is None:
        raise ValueError("KMS-backed HAIP bootstrap requires --haip-material")
    ...
    configure_haip_environment(
        environment,
        args.haip_material,
        require_request_object_trust=args.eudi,
    )
...
def stop_started(
    ...
    environment: dict[str, str],
    *,
    marty_include_haip: bool | None = None,
) -> None:
    down = dict(components(args, projects, "down"))
    if marty_include_haip is not None:
        down["marty"] = lambda: marty_command(
            args,
            projects,
            "down",
            include_haip=marty_include_haip,
        )
...
def execute(args: argparse.Namespace) -> int:
    ...
    environment = child_environment(
        require_haip=args.haip and args.command != "up",
    ...
    local_docker = docker_endpoint_is_local(environment)
    if not local_docker:
        validate_remote_bind_contract(args, environment)
        if args.command == "up" and args.haip and not environment.get("VERIFIER_X509_CERT_PEM", "").strip():
            raise ValueError(
                "remote Docker HAIP startup requires an externally issued certificate for the configured "
                "issuer-profile KMS key; in-run profile certification is local-runner only"
            )
    ...
    started: list[str] = []
    up_entries = selected
    externally_certified = bool(environment.get("VERIFIER_X509_CERT_PEM", "").strip())
    if args.haip and externally_certified:
        configure_haip_environment(
            environment,
            args.haip_material,
            require_request_object_trust=args.eudi,
        )
    if args.haip and not externally_certified:
        # Start migrations, KMS, gateway, and the issuer-profile registry before
        # asking the disposable test CA to certify the profile's public DID key.
        # Marty private key material never leaves the signing service/KMS path.
    ...
    for name, command in up_entries:
